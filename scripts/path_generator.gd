...
func _init():
	...
	#creates the path that gets created
	# makes sure the path the right size
...
# loop could be placed. Loops can contain loops within them, making for some
# interesting results
...
func _add_loops():
	...
	while loops_generated and _loop_count < path_config.max_loops:
		...
		for i in range(_path_route.size()):
			...
			# If the loops size > 0, then _is_loop_option found a loop so adds
			# it to the array
			if loop.size() > 0:
