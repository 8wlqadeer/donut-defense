...
func set_patrolling(patrolling: bool):
	$watermelonShape/PatrolZone.monitoring = patrolling
#rotation script
func rotate_towards_target(rtarget, delta):
	# gets the position of the target and rotates towards its target.
	var target_Vector = $watermelonShape.global_position.direction_to(Vector3(rtarget.global_position.x,global_position.y,rtarget.global_position.z))
	var target_basis: Basis = basis.looking_at(target_Vector)
	$watermelonShape.basis = $watermelonShape.basis.slerp(target_basis, acquire_slerp_progress)
...
func _on_acquiring_state_state_physics_processing(delta):
	if current_enemy != null and enemies_in_range.has(current_enemy):
		$watermelonShape.look_at(current_enemy.global_position)
	else:
...
func _maybe_fire():
	if weapon_type == "seed":
		if Time.get_ticks_msec()>(last_fire_time+fire_rate_ms):
			var projectile:Seed = projectile_type.instantiate()
			projectile.starting_position = $watermelonShape/projectile_spawn.global_position
...
func _on_extra_reload_upgrade_pressed():
	if GameManaging.money >= 100:
		if hit_reload == false:
			...
			$ExtraReloadUpgrade.text = "Upgrade Maxed"
func _on_frost_upgrade_pressed():
	if GameManaging.money >= 5000:
		$FrostUpgrade.text = "Upgrade Maxed"
		if hit_once == false:
			GameManaging.money -= 5000
			weapon_type = "frostySeed"
			hit_once = true
		else:
			$FrostUpgrade.text = "Upgrade Maxed"
