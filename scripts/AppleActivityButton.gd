...
# same script as WatermelonActivityButton
...
var tower_rotation:Vector3 = Vector3.ZERO
...
func _ready():
	tower_rotation = Vector3(0,180,0)
...
func _physics_process(_delta):
	if _is_dragging and GameManaging.money >= 300:
...
func _on_button_down():
	print(GameManaging.money)
	_is_dragging = true
	_is_valid_location = false

func _on_button_up():
	if GameManaging.money >= 300 and _is_valid_location == true:
		GameManaging.money -= 300
		print(GameManaging.money)
		no_money.visible = false
		GameManaging.score +=15
	elif _is_valid_location == false:
		action_object.visible = false
	else:
		no_money.text = "Not enough money, need " + str(300 - GameManaging.money)+ " more to buy"
